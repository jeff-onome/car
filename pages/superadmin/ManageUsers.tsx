...
const ManageUsers: React.FC = () => {
  ...
  const handleConfirmAction = () => {
    ...
    switch (modalState.action) {
      case 'approve':
        updateUser(modalState.user.email, { verificationStatus: 'Verified' });
        break;
      case 'reject':
        updateUser(modalState.user.email, { verificationStatus: 'Rejected' });
        break;
      case 'delete':
        if (modalState.user.role === 'dealer') {
          deleteCarsByDealer(modalState.user.email);
        }
        deleteUser(modalState.user.email);
        break;
      case 'block':
        updateUser(modalState.user.email, { status: 'Blocked' });
        break;
      case 'unblock':
        updateUser(modalState.user.email, { status: 'Active' });
        break;
    }
    closeModal();
  };
