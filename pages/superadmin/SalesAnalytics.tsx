...
const SalesAnalytics: React.FC = () => {
    ...
    const salesData = useMemo(() => {
        ...
        const dealerMap = new Map(dealers.map(dealer => [dealer.email, { ...dealer, salesCount: 0, totalValue: 0 }]));
                ...
                <div className="bg-secondary p-6 rounded-lg border border-border">
                    ...
                    <ul className="space-y-3">
                        {salesData.topDealersBySales.slice(0, 5).map(dealer => (
                            <li key={dealer.email} className="flex justify-between items-center text-sm">
                                <div>
